...
fast_time_series_catch_all <- function(data, f, h, trainStart, trainEnd, test, n, cols, bestModels){
  ...
  for (i in 2:length(data)){  #goes through all columns assume col 1 is the date field and skipping that one
    if (i %in% skip){  #made a skip vector to skip certain cols such as predictor variables that we do not want to predict, may be zero
     next()
    }
